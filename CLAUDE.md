...
- **Learning path is data-driven.** `LearningPath.stages(for: instrument)` (in `AppModel.swift`) is the content, and `AppModel.stages` returns the path for the current instrument: each stage has an ordered list of `Lesson`s, and stage 2 (and later stage 3) differ for guitar and bass. `AppModel` persists completed lessons and derives each stage's done/active/locked status and the overall percent, so a new user starts at 0% with stage 1 active, and finishing a stage's lessons unlocks the next. `StageLessonsView` (in `LessonView.swift`) plays a stage's lessons one screen each, resuming at the first unfinished lesson; a lesson advances with a Next / Finish button. Lessons render by `LessonKind`: `.explore` (a guided tap-to-hear fretboard), `.tab(Riff)` (the fretboard locked to a tab staff with Play / Stop, also tap-to-hear), and `.reading` (text). Stage 1 (Fretboard Basics) is three explore lessons and stage 2 (Tabs) is a five-lesson curriculum with separate guitar and bass tracks; stages 3-5 are still single `.tab(.drift)` stubs pending their own increments. `markLessonComplete(stageID:lessonID:)` is the hook lessons call to record completion.
