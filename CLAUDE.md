...
- **Learning path is data-driven.** `LearningPath.stages` (in `AppModel.swift`) is the five-stage content. `AppModel` stores per-stage completion and derives each stage's done/active/locked status and the overall percent, so a new user starts at 0% with stage 1 active. Lesson content is still stubbed: `LessonView` takes the tapped stage's number, title, and subtitle and shows one shared interactive fretboard/riff demo. Call `setStageProgress(_:forStage:)` to advance a stage when real lessons exist.
