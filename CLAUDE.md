...
- **Learning path is data-driven.** `LearningPath.stages(for: instrument)` (in `AppModel.swift`) is the content, and `AppModel.stages` returns the path for the current instrument: each stage has an ordered list of `Lesson`s, and stage 2 (and later stage 3) differ for guitar and bass. `AppModel` persists completed lessons and derives each stage's done/active/locked status and the overall percent, so a new user starts at 0% with stage 1 active, and finishing a stage's lessons unlocks the next. `StageLessonsView` (in `LessonView.swift`) plays a stage's lessons one screen each, resuming at the first unfinished lesson; a lesson advances with a Next / Finish button, or a handoff button that opens a tool tab (a `Lesson.handoff` sets `AppModel.selectedTab`, the `MainTab` bound to the main `TabView`). Lessons render by `LessonKind`: `.explore` (a guided tap-to-hear fretboard), `.tab(Riff)` (the fretboard locked to a tab staff with Play / Stop, also tap-to-hear), `.scale(key:type:)` (a scale on the neck with the root in cyan and degree labels, tap-to-hear), `.backing(key:type:)` (the Solo screen's neck inside a lesson, the key's safe notes lit with the active backing-chord root pulsing as the backing loop plays, tap-to-hear, with a Play / Stop backing transport in the footer), and `.reading` (text). Stage 1 (Fretboard Basics) is three explore lessons, stage 2 (Tabs) is a five-lesson curriculum with separate guitar and bass tracks, stage 4 (Scales & Keys) is five scale lessons ending in a handoff to the Scale Explorer, and stage 5 (Improvisation) is a five-lesson curriculum (a static safe-notes scale lesson followed by four backing-loop lessons) ending with a handoff to Solo Practice; only stage 3 (Chords) remains a single `.tab(.drift)` stub. `markLessonComplete(stageID:lessonID:)` is the hook lessons call to record completion.
